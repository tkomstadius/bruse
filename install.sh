PASSWORD=$(date +%s | sha256sum | base64 | head -c 32 ; echo)
{
  ...
  echo "Error trying to create databse credentials. Make sure Postgres is installed and that the user with admin rights is named postgres."
  exit
}
{
  curl -L https://github.com/tkomstadius/bruse/archive/develop.tar.gz | tar xz --strip-components=1
} || {
  echo "Have you installed curl?"
  exit
}
